...
from src.core.migrate.claim_rpt.data_type.claim_data_type import (
    CLAIM_MIGRATE_COLS,
)
        ...
        for file in all_files:
            ...
            try:
                ...
                for i, chunk in enumerate(df):
                    print(
                        f"=========== [START] Loading provider claims of batch {i + 1} ==========="
                    )
                    ...
                    claim_df = (
                        ...
                        .drop_duplicates(
                            subset=["CLAIM_ID", "DIAGNOSIS_CODE"], keep="first"
                        )
                    ...
                    for row in claim_df.to_dict(orient="records"):
                        ...
                        service_lines_from_ardb = (
                            service_line_df[
                                service_line_df["CLAIM_ID"] == ardb_claim_id
                            ]
                        ...
                        if provider_claim_from_db is None:
                            _id = generate_uuid()
                            provider_claim_from_inserted_list: (
                                ITherapyProviderClaim | None
                            ) = None
                            provider_claim_inserted_index: int = -1

                            # search in inserted provider claims
                            for index, provider_claim in enumerate(
                                inserted_provider_claims
                            ):
                                if provider_claim["CLAIM_ID"] == ardb_claim_id:
                            ...
                            service_lines_from_inserted_list: list(
                                IProviderClaimServiceLine
                            ) = (
                                []
                                if provider_claim_from_inserted_list is None
                                else provider_claim_from_inserted_list.get(
                                    "SERVICE_LINES", []
                                )
                            )

                            # only map claims information
                            provider_claim_therapy_format = provider_claim_mapper.to_therapy_claim_format(
                                provider_claim=row,
                                _id=_id,
                            ...
                            # map service lines
                            mapped_service_lines_response = provider_claim_mapper.to_therapy_service_line_format(
                        ...
                        if provider_claim_from_db is not None:
                            ...
                            service_lines_from_updated_list: list(
                                IProviderClaimServiceLine
                            ) = (
                                []
                                if provider_claim_from_updated_list is None
                                else provider_claim_from_updated_list.get(
                                    "SERVICE_LINES", []
                                )
                            )
                            ...
                            mapped_service_lines_response = (
                                provider_claim_mapper.to_therapy_service_line_format(
                                    ...
                                    old_provider_service_lines=(
                                        ...
                                        if provider_claim_from_updated_list is not None
                                        else provider_claim_from_db.get(
                                            "SERVICE_LINES", []
                                        )
                                    ),
                                )
